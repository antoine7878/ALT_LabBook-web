...
export const addCompound = async (
  ...
  return await ctx.prisma.compound.create({
    data: {
      ...
      index: compoundNumber
    },
  });
};

export const setNameCompound = async (
  _: any,
  args: { id: string; name: string },
  ctx: Context
) => {
  return await ctx.prisma.compound.update({
      where: {
        id: Number(args.id),
      },
      data: {
...
export const setImgStructCompound = (
  _: any,
  args: { id: string; img: string; struct: string },
  ctx: Context
) => {
...
export const moveCompound = async (
  _: any,
  args: { movingCompoundId: string, destinationGroupId: string, destinationPosition: string },
  ctx: Context
) => {
  const pos = Number(args.destinationPosition)
  let destCompounds = (await ctx.prisma.compound.findMany({ where: { groupId: Number(args.destinationGroupId), } })).sort((a, b) => a.index - b.index).filter(c => c.id !== Number(args.movingCompoundId))

  let updated = false
  let index = 0
  for (let compound of destCompounds) {
    const id = compound.id
    if (index === pos) {
      //update the moving compound
      await ctx.prisma.compound.update({
        where: {
          id: Number(args.movingCompoundId)
        },
        data: {
          index,
          group: { connect: { id: Number(args.destinationGroupId) || 1 } },
        }
      })
      updated = true
      index++
